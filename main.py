...
st.text('This Application will help you download MSI, NDVI and EVI with one click for your Region of Interest with Interactive Visualisation')
st.divider()

st.caption('Upload your Region of Interest as KML or Geopackage or GeoJSON')

uploaded_file = st.file_uploader(label = 'Selecte the file below 👇', 
                 type = ['kml', 'gpkg', 'geojson'])

if not uploaded_file is None:
    
    filename = uploaded_file.name
    st.badge(f'Succesfully uploaded file: {filename}')
    ...
    st.caption('Please select the Date Range for your Region of Interest')
    ...
    if end_date_format != 'None':

        st.badge(f'Selected Date Range is: {start_date} to {end_date}', color = 'green')
